  ...
  try {
    ...
    if (!projectId || !revisionPrompt || !baseVersionId) {
      return NextResponse.json(
        { error: 'Project ID, revision prompt, and base version ID are required' },
        { status: 400 }
      )
    }
    ...
    if (typeof revisionPrompt !== 'string' || revisionPrompt.trim().length === 0) {
      return NextResponse.json(
        { error: 'Revision prompt must be a non-empty string' },
        { status: 400 }
      )
    }
...
<body>
    <div class="container">
      ...
      return NextResponse.json(
        { error: 'Failed to generate website revision. Please try again.' },
        { status: 500 }
      )
    }
